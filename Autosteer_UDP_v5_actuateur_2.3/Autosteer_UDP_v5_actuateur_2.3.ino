    ...
    * 
    * june 2023  add  some   function with two relay to  activate   electric  cylinder  and   do smart   rotation   for the gear 
    */
    ...
    */
    ...
    #define R1 2  //Add   here
    #define R2 5  //Add   here
    ...
    uint32_t time1 = 0 ;   //Add   here
    uint32_t time2 = 0 ;//Add   here
    uint32_t time3 = 1200 ;//Add   here
    uint32_t time4 = 300 ;//Add   here
  ...
  void loop()
  {
      ...
      if (currentTime - lastTime >= LOOP_TIME)
      {
          ...
          //reset debounce
          encEnable = true;
if (currentTime >(time1+time3))   digitalWrite(R1, LOW);   //Add   here
          ...
          if (steerConfig.CurrentSensor)
          {
              ...
              sensorReading = abs (steerAngleActual - steerAngleBefore)*10;
              
       steerAngleBefore =   steerAngleActual;

              if (sensorReading >= steerConfig.PulseCountMax)
              if ((pwmDrive > 50) & (( sensorReading / pwmDrive * 50 ) >= steerConfig.PulseCountMax))
     //  if (abs (steerAngleActual - steerAngleBefore) > steerConfig.PulseCountMax / 10 )
              
              {
                  ...
                  previous = 0;
                    ...
                    digitalWrite(R1, HIGH);   //Add   here
                    digitalWrite(R2, HIGH);

                    time1 = millis();   //Add   here
...
//                  Serial.print(", ratio ");
//                  Serial.print(sensorReading / pwmDrive * 50 );
//                  
